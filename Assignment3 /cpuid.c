...
//assg2 changes ----------------------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------------------------
atomic64_t totalExits = ATOMIC64_INIT(0);
...
//------------------------------------------------------------------------------------------------------------------------------------
	...
	{ 
		if(ecx == 35 || ecx == 38 || ecx == 42 || ecx == 65 || ecx>68 || ecx <0){
			printk(KERN_INFO "The exit reason number of %u is not defined in SDM", ecx);
			eax = 0;
			ebx = 0;
			ecx = 0;
			edx = 0xffffffff; } 
			...
			eax = atomic64_read(&totalExits);
			printk(KERN_INFO "########## Total number of exits = %u", eax);  } 
	} 

	else if(eax == 0x4ffffffd) 
	{ 
		if(ecx == 35 || ecx == 38 || ecx == 42 || ecx == 65 || ecx>68 || ecx<0) { 
			printk(KERN_INFO "The exit reason number of %u not defined in SDM", ecx);
			eax = 0;
			ebx = 0; 
			ecx = 0;
			edx = 0xffffffff;  } 
			...
			printk("Total cycles spent in exit = %llu", atomic64_read(&totalTime));
			ebx = (atomic64_read(&totalTime) >> 32);
			printk("The High 32 bits of total time spent processing exits in ebx = %d", ebx);
